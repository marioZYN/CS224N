...
class CharDecoder(nn.Module):
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        char_pad_idx = self.target_vocab.char2id['<pad>']        
        loss = nn.CrossEntropyLoss(ignore_index=char_pad_idx, reduction='sum')
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        cur = torch.tensor([[self.target_vocab.start_of_word]*batch_size], device=device) # (1, b)
        ht, ct = initialStates
        preds = []
        for _ in range(max_length):
            scores, (ht, ct) = self.forward(cur, (ht, ct)) # scores (1, batch, V)
            scores = nn.functional.softmax(scores.squeeze(0), 1)
            cur = torch.argmax(scores, 1).unsqueeze(0) # (1, b)
            chars = [self.target_vocab.id2char[x.item()] for x in cur[0]]
            preds.append(chars)
