...
# self write sanity check
def question_1h_sanity_check():
    """ Sanity check for Highway model
        basic shape check
    """
    print ("-"*80)
    print ("Running Sanity Check for Question 1h: Highway network")
    print ("-"*80)
    BATCH_SIZE, EMBED_SIZE = 4, 5
    model = Highway(EMBED_SIZE, EMBED_SIZE)
    x = torch.zeros(BATCH_SIZE, EMBED_SIZE)
    y = model(x)
    assert(y.shape == x.shape), "Output shape of Highway should be {} but is: {}".format(x.shape, y.shape)
...
def question_1i_sanity_check():
    ...
    print('-'*80)
    BATCH_SIZE, IN_DIM, OUT_DIM, K, M_WORD = 10, 4, 8, 3, 5
    model = CNN(IN_DIM, OUT_DIM, K)
    x = torch.zeros(BATCH_SIZE, IN_DIM, M_WORD)
    y = model(x)
    assert(list(y.shape) == [BATCH_SIZE, OUT_DIM, 1]), "Output shape of CNN should be {} but is: {}"\
        .format((BATCH_SIZE, OUT_DIM, 1), y.shape)
