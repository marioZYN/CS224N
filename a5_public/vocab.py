...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids = [[[self.start_of_word]+[self.char2id.get(c, self.char_unk) for c in word]+[self.end_of_word] for word in sent] for sent in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # (bach_size, max_sentence_length, max_word_length)
        sents_var = pad_sents_char(sents_var, self.char2id['<pad>'])
        sents_var = torch.tensor(sents_var, dtype=torch.long, device=device).permute(1, 0, 2)
    ...
    def from_corpus(corpus, size, freq_cutoff=2):
        ...
        word_freq = Counter(chain(*corpus)) # I think Counter(corpus) is enough
