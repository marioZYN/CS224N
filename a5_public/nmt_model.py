...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # Tensor : (src_len, b, m)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device) # Tensor : (tgt_len, b, m)
        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device)   # Tensor: (tgt_len, b)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(packed_out, padding_value=self.vocab.src.word2id['<pad>'], batch_first=True)
        # last hidden, h_n: (num_layers * num_directions, batch, hidden_size)
        last_hidden_forward = h_n[0,:]
        last_hidden_backward = h_n[1,:]
        last_hidden = torch.cat([last_hidden_backward, last_hidden_forward], 1)
        # last cell, c_n: (num_layers * num_directions, batch, hidden_size)
        last_cell_forward = c_n[0,:]
        last_cell_backward = c_n[1,:]
        last_cell = torch.cat([last_cell_backward, last_cell_forward], 1)
        # init_decoder_hidden, (batch, hidden_size)
        init_decoder_hidden = self.h_projection(last_hidden)
        # init_decoder_cell, (batch, hidden_size)
